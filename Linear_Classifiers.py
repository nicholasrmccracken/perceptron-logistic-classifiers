...
def Logistic_Regression(X, Y, learningRate=0.01, maxIter=100):
    ...
    for t in range(maxIter):
        predictions = sigmoid(np.matmul(w.T, X)).T

        gradient = np.matmul(X, (predictions - Y)) / N
...
def Perceptron(X, Y, learningRate=0.01, maxIter=100):
    ...
    for t in range(maxIter):
        ...
        for n in range(N):
            prediction = np.sign(np.matmul(w.T, X[:, n].reshape(-1, 1)))

            if prediction != Y[n]:
                w += learningRate * Y[n] * X[:, n].reshape(-1, 1)
